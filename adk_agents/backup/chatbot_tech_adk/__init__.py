"""
chatbot_tech_adk - 기술지원 상위 챗봇 (L1 Parent Agent)
"""
...
agent = Agent(
    ...
    3. 신뢰도가 70% 미만이거나, 세부 기술 상담이 필요한 경우 하위 전문가 호출을 제안하세요

    하위 전문가 목록:
    - chatbot-tech-backend: 백엔드 개발 전문가 (API, DB, 서버 등)
    - chatbot-tech-frontend: 프론트엔드 개발 전문가 (UI, UX, React 등)
    ...
    답변은 한국어로 작성하세요.
    """,
    description="기술 관련 모든 문의를 처리하는 상위 챗봇. 세부 사항은 하위 전문가에게 위임"
)
