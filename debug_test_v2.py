"""
ADK 디버그 환경 검증 스크립트 v2 (ADK 1.18.0 호환)
"""
import os
import sys
import logging
...
# 2. SessionService 확인 (비동기)
print("\n2. SessionService 검증 (비동기)...")
async def test_session_service():
    try:
        ...
        print(f"   ✅ 세션 생성 성공")
        print(f"   📋 세션 ID 속성: {getattr(session, 'id', 'N/A')}")
        print(f"   📋 세션 타입: {type(session)}")
        print(f"   📋 세션 attributes: {dir(session)}")
        return True
...
result = asyncio.run(test_session_service())
